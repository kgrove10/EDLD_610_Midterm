...
```{r effect_size_all}
# We successfully applied the code you provied, and it worked perfectly
by_school_grade_year <- school_data %>% 
  group_by(school, grade_level, academic_year) %>% 
  nest()

# We next added gaps to the above dataframe in one line and recoded grade_level for common syntax
...
# The dataframe came out looking as expected, with school, grade level, academic year, and gap included     
head(gaps)
```
...
```{r plots}
# Next, we take the dataframe created above and slice it to the first 100 cases
gaps_slice <- gaps %>% 
  slice(1:100) %>% 
  as.data.frame() # Here is where we run into issues - when we try and create plots for each of the cases, R spits out the following error: `data` must be a data frame, or other object coercible by `fortify()`, not a character vector. We tried coercing the data to a dataframe, but to no avail. The same error came up.

str(gaps_slice) # `str` shows that the object we created is a dataframe

lapply(gaps_slice, function(x) {
    ggplot(x, aes(grade_level, academic_year, fill = gaps)) +
    coord_flip() +
    geom_bar(stat = "identity")
})

# We also tried creating another new object that excluded the nested data for academic level among white and Hispanic/Latinx students. Perhaps that was the issue?
test_gaps <- gaps_slice %>%
  select(-data)

# Alas, the same error returned
lapply(test_gaps, function(x) {
    ggplot(x, aes(grade_level, academic_year, fill = gaps)) +
    coord_flip() +
    geom_bar(stat = "identity")
})
